...
def cme_activity_score_DAY0():
    spark=session.get_spark_session()
    request_id = pipeline_audit.start_audit(pipeline_stage='SILVER_TO_GOLD',pipeline_target_table='cme_activity_score',spark=spark)
    ...
    df = spark.sql(f"""
        WITH A as (
            ...
            where ca.is_most_accurate=true
            ),
            B as (
            select 
            q.cme_id,
