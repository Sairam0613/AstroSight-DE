...
def Neo_Rankings():
    ...
    df = spark.sql(f"""
        with chng as (
        select Asteroid_Id from {iceberg_catalog}.{silver_layer}.neo_objects where ingestion_timestamp >=(select max(t.pipeline_end_time) as last_refresh 
                ...
                and t.pipeline_target_table='neo_rankings')
        UNION
        select Asteroid_Id from {iceberg_catalog}.{silver_layer}.neo_close_approaches where ingestion_timestamp >=(select max(t.pipeline_end_time) as last_refresh 
                from {iceberg_catalog}.{bronze_layer}.pipeline_audit t where t.pipeline_stage_status='PASSED'
                and t.pipeline_stage='SILVER_TO_GOLD'
                and t.pipeline_target_table='neo_rankings')
        ),
        ...
        on no.asteroid_id = ca.asteroid_id
        where no.Asteroid_Id in (select Asteroid_Id from chng)) ranked
